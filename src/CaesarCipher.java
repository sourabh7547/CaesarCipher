...
public class CaesarCipher{
    public static String Encrypt(String text, int positions){
        ...
        //forward the text to be encrypted
        for (int i =0; i<text.length(); i++){
            ...
            if (text.charAt(i)== ' ')
                continue;
                 ...
                 //if the character is lowercase, mode it uppercase
        ...
        //the encrypted code is a already uppercase,
            //therefore there is no need forwarding
            String result = "";
            for (int i = 0; i < text.length(); i++){

                //shift the current letter of the message with given positions to left
                char shiftedLetter = (char) (text.charAt(i) - positions);
                //if the ASCII code exceeds A, then bring ot back in the interval A...Z
                if (shiftedLetter < 'A')
                    shiftedLetter = (char) (shiftedLetter - 'A' + 'Z' +1);
    ...
    public static void main(String[] args) {
        ...
        // Decrypted the encrypted message
